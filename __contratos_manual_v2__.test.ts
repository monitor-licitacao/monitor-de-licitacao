...
import { mapTenantContractInsertError } from './server/lib/contratos/errors.js';
...
test('manual v2 — unique PNCP vira mensagem amigável', () => {
  const err = mapTenantContractInsertError(
    new Error('duplicate key value violates unique constraint "tenant_contract_pncp_unique"'),
  );
  assert.equal(err.message, 'Este contrato já está no painel.');
});
...
test('manual v2 — resolveOrgaoCnpjFromContratacaoMeta fallback PNCP', () => {
  assert.equal(
    resolveOrgaoCnpjFromContratacaoMeta({
      cnpj_orgao: '',
      numero_controle_pncp: '03612122000127-1-000026/2026',
