# Notas Práticas de Deploy (Hetzner VPS)

Passo a passo para colocar o Monitor de Licitações rodando na mesma VPS Hetzner que já hospeda o `gymsite-api` (CX33, 4 vCPU / 8 GB — confirmado com folga de \~6,3 GB de RAM e 34 GB de disco livres em 04/09/2026), reaproveitando o túnel Cloudflare já existente em vez de abrir portas novas.
...
Isso cria `~/.ssh/licitacoes_deploy_key` (privada) e `~/.ssh/licitacoes_deploy_key.pub` (pública). Usar uma chave própria pro GitHub Actions (em vez da sua pessoal) significa que, se um dia precisar revogar o acesso do CI, você não perde o seu próprio acesso.
...
Isso limita o que o GitHub Actions consegue fazer na VPS: o usuário `deploy` só tem permissão de usar Docker (via grupo `docker`), não é root pleno.
...
Copie para `/opt/licitacoes/` os arquivos `docker-compose.licitacoes.yml` deste repositório, e crie `/opt/licitacoes/.env.production` com:
...
Edite (como root) `/opt/gymsite/cloudflared/config.yml` e adicione uma entrada **antes** da linha `- service: http_status:404` (a ordem importa — o Cloudflare Tunnel usa a primeira regra que casar):
...
A partir daí, todo push na `main` builda a imagem, publica em `ghcr.io/monitor-licitacao/monitor-de-licitacao` e faz o deploy automático via SSH, sem downtime perceptível fora da janela de troca do container.
