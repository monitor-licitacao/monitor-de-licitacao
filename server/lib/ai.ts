...
export async function checkOllamaHealth(timeoutMs = 1500): Promise<{ ok: boolean; models: string[]; error?: string }> {
  const rawBase = (process.env.OLLAMA_BASE_URL || 'http://127.0.0.1:11434/v1').replace(/\/v1\/?$/, '');
  const validation = validateAllowedOutboundUrl(rawBase, OLLAMA_OUTBOUND_URL_POLICY);
  if (validation.ok === false) {
    return { ok: false, models: [], error: validation.reason };
  }

  const apiKey = process.env.OLLAMA_API_KEY;
  const headers: Record<string, string> = {};
  if (apiKey && apiKey !== 'ollama' && hostnameMatchesAllowlist(validation.url.hostname, 'ollama.com')) {
    headers.Authorization = 'Bearer ' + apiKey;
  }

  const tagsUrl = new URL('/api/tags', validation.url).toString();
