...
import { mapTenantContractInsertError } from './errors.js';

export { resolveOrgaoCnpjFromContratacaoMeta } from './map-contratacao-items.js';
export { stripCnpj };
...
async function computeDesvioPercentual(
  _row: TenantContractRow,
  _items: TenantContractItemRow[],
): Promise<number | null> {
  // Histórico de preço (#86 / 0005) fica fora deste recorte — fator neutro.
  return null;
}
...
async function persistManualContractRow(input: {
  ...
  try {
    ...
    throw mapTenantContractInsertError(err);
  }
