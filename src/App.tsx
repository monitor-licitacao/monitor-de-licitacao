...
function isPipelinePath(): boolean {
  if (typeof window === 'undefined') return false;
  return window.location.pathname === '/pipeline' || window.location.pathname.startsWith('/pipeline/');
}
...
export default function App() {
  ...
  const handleParticiparContratacao = async (contratacaoId: string) => {
    try {
      const res = await apiClient('/api/pipeline', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contratacao_id: contratacaoId }),
      });
      if (res.status === 409) {
        showToast('Esta licitação já está no seu pipeline.', 'info');
        setActiveTab('pipeline');
        window.history.pushState(null, '', '/pipeline');
        return;
      }
      await assertOk(res);
      showToast('Licitação adicionada ao pipeline!');
      setActiveTab('pipeline');
      window.history.pushState(null, '', '/pipeline');
    } catch (err: unknown) {
      showToast(err instanceof Error ? err.message : 'Erro ao participar', 'error');
    }
  };
  ...
  const handleNavigateTab = (tab: string) => {
    ...
    if (tab === 'contratacoes') {
      ...
      setSelectedContratacaoId(null);
      setSelectedContratoId(null);
      if (!window.location.pathname.startsWith('/pipeline')) {
        window.history.pushState(null, '', '/pipeline');
      }
    } else if (window.location.pathname.startsWith('/contratacoes')) {
      setSelectedContratacaoId(null);
      ...
      setSelectedContratoId(null);
      window.history.pushState(null, '', '/');
    } else if (window.location.pathname.startsWith('/pipeline')) {
      window.history.pushState(null, '', '/');
    }
    setActiveTab(tab);
  };
          ...
          <main className="flex-1 overflow-y-auto no-scrollbar pb-10">
            ...
            {activeTab === 'contratacoes' && (
              <ContratacoesView
                ...
                onParticipar={handleParticiparContratacao}
              />
            )}

            {activeTab === 'pipeline' && (
              <PipelineView
                onNavigateContratacoes={() => handleNavigateTab('contratacoes')}
                onOpenContratacao={(id) => handleSelectContratacaoId(id)}
              />
            )}
