...
import { isContratacaoAnulada } from '../../lib/contratacaoAnulada';
...
export const ContratacaoDetailView: React.FC<ContratacaoDetailViewProps> = ({
  ...
  const anulada = isContratacaoAnulada({
        ...
        <div className="p-5 sm:p-6 space-y-4 border-b border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {onParticipar ? (
              <button
                ...
                title={
                  anulada
                    ? 'Contratação anulada — não entra no pipeline'
                    : 'Adicionar ao pipeline de participação'
                }
                onClick={() => {
                  if (anulada) return;
                  onParticipar(contratacaoId);
                }}
                className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-semibold border ${
                  anulada
            ...
            {onRegisterContrato ? (
              <button
                ...
                title={
                  anulada
                    ? 'Origem anulada: o contrato entra só como arquivo histórico'
                    : registerLabel
                }
                onClick={() =>
                  onRegisterContrato({
          ...
          {anulada ? (
            <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              Contratação anulada no PNCP. Participar fica bloqueado. Registrar cria só um arquivo
              histórico — o contrato não entra na distribuição por saúde do Raio-X.
            </p>
          ) : null}
