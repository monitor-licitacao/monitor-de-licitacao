...
export const ContratoManualModal: React.FC<ContratoManualModalProps> = ({
  ...
  const [orgaoTone, setOrgaoTone] = useState<'ok' | 'local'>('local');
  ...
  const applyDetail = useCallback(
    (
      ...
      if (cnpj.replace(/\D/g, '').length === 14) {
        void apiClient(`/api/contratos/cnpj/${cnpj.replace(/\D/g, '')}`)
          .then(async (res) => {
            if (!res.ok) return;
            const payload = (await res.json()) as {
              data?: { fonte?: string; razao_social?: string; provider?: string };
            };
            const fonte = payload.data?.fonte;
            if (fonte === 'brasilapi' || fonte === 'receitaws') {
              setOrgaoTone('ok');
              setOrgaoMsg(
                `Órgão encontrado (${fonte === 'receitaws' ? 'ReceitaWS' : 'BrasilAPI'}): ${payload.data?.razao_social ?? orgaoNome}`,
              );
            }
          })
          .catch(() => undefined);
        ...
        <div className="p-4 space-y-4">
          ...
          {selectedSource && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
              ...
              {orgaoMsg && (
                <p className={`text-xs mt-2 ${orgaoTone === 'ok' ? 'text-emerald-700' : 'text-amber-700'}`}>
                  {orgaoMsg}
                </p>
              )}
            </div>
