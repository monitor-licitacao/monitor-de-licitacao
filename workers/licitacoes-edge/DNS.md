...
## Se for ativar o Worker no hostname público

O CI faz `wrangler deploy` **sem** Custom Domain (fica em `*.workers.dev`).
...
2. Em `wrangler.jsonc`, recolocar:

   ```jsonc
   "routes": [{ "pattern": "licitacoes.getgymsite.com.br", "custom_domain": true }]
   ```

3. `npm run deploy` em `workers/licitacoes-edge`
   → Cloudflare cria o DNS do Custom Domain apontando ao Worker
4. Manter `licitacoes-origin.getgymsite.com.br` → túnel (`ORIGIN_URL`)
